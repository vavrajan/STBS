...
with open(os.path.join(slurm_dir, 'run_all_PF_review.slurm'), 'w') as all_file:
    ...
    for i in addenda:
        addendum = str(i)
        for K in Ks:
            flags = '  --data='+data_name+'  --num_topics='+str(K)+'  --addendum='+addendum+'_review'
            pf_path = os.path.join(pf_dir, 'PoisFact_'+addendum+"_K"+str(K)+'_review'+'.slurm')
            with open(pf_path, 'w') as file:
                file.write('#!/bin/bash\n')
                file.write('#SBATCH --job-name=PF_' + data_name + '_review' + ' # short name for your job\n')
