...
for i in addenda:
    addendum = str(i)
    for K in Ks:
        name = "TBIP_" + addendum + "_K" + str(K) + '_review'
...
for i in addenda:
    ...
    for K in Ks:
        for covariates in ["party", "all_no_int", "all"]:
            name = "STBS_ideal_a_" + covariates + addendum + "_K" + str(K) + '_review'
            scenarios[name] = {"checkpoint_name": name,
                               "addendum": addendum,
                               "num_topics": K,
                               "computeIC_every": computeIC_every,
...
for i in addenda:
    ...
    for K in Ks:
        for covariates in ["party", "all_no_int", "all"]:
            name = "STBS_ideal_ak_" + covariates + addendum + "_K" + str(K) + '_review'
            scenarios[name] = {"checkpoint_name": name,
                               "addendum": addendum,
                               "num_topics": K,
                               "computeIC_every": computeIC_every,
...
with open(os.path.join(slurm_dir, 'run_all_STBS.slurm'), 'w') as all_file:
    ...
    for name in scenarios:
        ...
        with open(stbs_path, 'w') as file:
            file.write('#!/bin/bash\n')
            file.write('#SBATCH --job-name=STBS_' + data_name+ '_review' + ' # short name for your job\n')
