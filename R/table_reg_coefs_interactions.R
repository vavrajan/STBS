...
for(k in 1:K){
  ...
  con <- file(paste0(tab_dir, "regression_coefs_k_",k-1,".tex"), open = "wt", encoding = "UTF-8")
  sink(con)
  cat(LTAB)
  sink()
  close(con)
...
topics <- c(6,10)
{
  ...
  con <- file(paste0(tab_dir, "regression_coefs_k_",paste(topics-1, collapse = "_"),".tex"), 
